...
String C1 = new String("");//for the contact classification
String N1 = new String("");//for the contact name
String I1 = new String("");//for the contact IFF category
...
//####### - begin CONTACT IFF button group - #######
      ...
      unknown.setPreferredSize(new Dimension(77, 25));
      ...
      friendly.setPreferredSize(new Dimension(77, 25));
      friendly.setMinimumSize(new Dimension(77, 25));
      friendly.setMaximumSize(new Dimension(77, 25));
      ...
      hostile.setPreferredSize(new Dimension(77, 25));
      ...
      if (c.getIffCategory() == 1) {
         friendly.setSelected(true);
         I1 = new String("FRIENDLY");
      } else if (c.getIffCategory() == 2) {
         hostile.setSelected(true);
         I1 = new String("HOSTILE");
      } else if (c.getIffCategory() == 3) {
         neutral.setSelected(true);
         I1 = new String("NEUTRAL");
      } else {
         unknown.setSelected(true);
         I1 = new String("UNKNOWN");
      } // end if-else
//####### - begin CONTACT TYPE button group - #######
      ...
      if (c.getClassification() == 2) {
          air.setSelected(true);
          C1 = new String("AIR");
      } else if (c.getClassification() == 1) {
...
//############################################################################
    ...
    contactInformation.add(typeInput, new GridBagConstraints(1, 5, 2, 1, 1.0, 0.0
            ,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(12, 0, 0, 0), 81, 0));
    ...
    jPanel1.add(jLabel3, new GridBagConstraints(3, 3, 1, 1, 0.0, 0.0
      ...
      double latMins = 0.0;
      if (lat >= 0) {
          if ((Math.ceil(lat) - lat) < 0.0001) {
              latDeg = (int) Math.abs(Math.ceil(lat));
              latComboBox.setSelectedIndex(0);
          } else {
              latDeg = (int) Math.abs(Math.floor(lat));
              latComboBox.setSelectedIndex(0);
          } // end if-else
          latMins = (lat - Math.floor(lat)) * 60.0;
      } else {
          if (Math.abs(Math.floor(lat) - lat) < 0.0001) {
              latDeg = (int) Math.abs(Math.floor(lat));
              latComboBox.setSelectedIndex(1);
          } else {
              latDeg = (int) Math.abs(Math.ceil(lat));
              latComboBox.setSelectedIndex(1);
          } // end if-else
          latMins = (Math.abs(lat) - Math.floor(Math.abs(lat))) * 60.0;
      } // end if-else
      if (Math.abs(latMins - 60.0) < 0.0001) {
          latMins = 0.0;
      } // end if
      String latDegrees = String.valueOf(latDeg);
      ctcDegreeLatitude.setText(latDegrees);
      DecimalFormat minutesFormat = new DecimalFormat("00.0");
      ctcMinuteLatitude.setText(minutesFormat.format(latMins));
      double longitude = c.getLocation().getLongitude();
      int longDeg = 0;
      double longMins = 0.0;
      if (longitude >= 0) {
          if ((Math.ceil(longitude) - longitude) < 0.0001) {
              longDeg = (int) Math.abs(Math.ceil(longitude));
              longComboBox.setSelectedIndex(0);
          } else {
              longDeg = (int) Math.abs(Math.floor(longitude));
              longComboBox.setSelectedIndex(0);
          } // end if-else
          longMins = (longitude - Math.floor(longitude)) * 60.0;
      } else {
          if (Math.abs(Math.floor(longitude) - longitude) < 0.0001) {
              longDeg = (int) Math.abs(Math.floor(longitude));
          ...
          longMins = (Math.abs(longitude) - Math.floor(Math.abs(longitude))) * 60.0;
      } // end if-else
      if (Math.abs(longMins - 60.0) < 0.0001) {
          longMins = 0.0;
      } // end if
      String longDegrees = String.valueOf(longDeg);
      ctcDegreeLongitude.setText(longDegrees);
      ctcMinuteLongitude.setText(minutesFormat.format(longMins));
      DecimalFormat headingFormat = new DecimalFormat("000.0");
      ctcCourse.setText(headingFormat.format(c.getVelocity().getHeading()));
      DecimalFormat tenthsFormat = new DecimalFormat("0.0");
      ctcSpeed.setText(tenthsFormat.format(c.getVelocity().getSpeed()));
      ctcAltitude.setText(tenthsFormat.format(c.getLocation().getAltitude()));
      DecimalFormat twoDigits = new DecimalFormat("00");
      ctcTimeHr.setText(String.valueOf(c.getLastReport().getHours()));
      newContactTime.setHours(c.getLastReport().getHours());
      ctcTimeMin.setText(String.valueOf(c.getLastReport().getMinutes()));
      newContactTime.setMinutes(c.getLastReport().getMinutes());
      ctcTimeSec.setText(String.valueOf(c.getLastReport().getSeconds()));
      newContactTime.setSeconds(c.getLastReport().getSeconds());
